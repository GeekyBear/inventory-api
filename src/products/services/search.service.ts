...
export class SearchService {
    ...
    async searchProducts(searchDto: SearchProductsDto): Promise<PaginatedResult<ProductResponseDto>> {
        const {
            page = 1,
            limit = 10,
            sort = 'relevance',
        ...
        switch (sortField) {
            case 'relevance':
                ...
                if (q) {
                    ...
                    sortObj = { score: { $meta: 'textScore' } as any, isFeatured: -1, createdAt: -1 };
                } else {
        ...
        return {
            data: products.map(product => new ProductResponseDto(product, product.category)),
            pagination: {
                page,
                limit,
                total,
