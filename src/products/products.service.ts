...
export class ProductsService {
    ...
    async findAll(paginationDto: PaginationDto & Partial<SearchProductsDto>): Promise<PaginatedResult<ProductResponseDto>> {
        ...
        return {
            data: products.map(product => new ProductResponseDto(product, product.categoryId)),
            pagination: {
                page,
                limit,
                total,
    ...
    async findByCategory(categoryId: string, paginationDto: PaginationDto): Promise<PaginatedResult<ProductResponseDto>> {
    ...
    async findLowStock(paginationDto: PaginationDto): Promise<PaginatedResult<ProductResponseDto>> {
        const { page = 1, limit = 10, sort = 'quantity', order = 'asc' } = paginationDto;
        const skip = (page - 1) * limit;
